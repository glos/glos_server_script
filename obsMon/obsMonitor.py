...
def compose(name,lastupdate,freq):
    ...
    template[1]="Buoy {0} might be offline, last report was at: {1} ({2} minutes ago) and has missed {3} report(s) at frequency of {4} minutes".format(name,ts.astimezone(et), round(timediff,2), int(timediff/(freq/60)),freq/60)
    return "\n".join(template)
